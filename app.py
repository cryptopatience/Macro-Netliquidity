...
```
    ...
    with tab_guide1:
        ...
        with col1:
            st.markdown("""
            **Standard 분석 (빠른 인사이트)**
            - **종합분석**: 전체 시장 상황 요약 (3-4분)
            - **유동성분석**: Fed 정책 방향 (2-3분)
            - **달러분석**: 달러 강도와 영향 (2-3분)
        ...
        with col2:
            st.markdown("""
            **Deep Dive 분석 (전문적 분석)**
            - **종합분석**: 7가지 관점 심층 분석 (10-15분)
            - **트레이딩전략**: 구체적 진입/청산가 (10-15분)
            - **신용분석**: 리스크 매트릭스 (8-12분)
    ...
    with tab_guide2:
        ...
        **1. 다중 시나리오 분석**
        - Bull Case / Base Case / Bear Case 제시
        - 각 시나리오별 확률과 대응 전략
        - 구체적인 가격 타겟 제시
    ...
    # 사용자 입력
    if prompt := st.chat_input("질문을 입력하세요 (예: 지금 비트코인 사도 될까요?)"):
        # 사용자 메시지 추가
...
<div style='text-align: center; color: gray;'>
    <p>🤖 FRED API 퀀트 3콤보 대시보드 + Gemini AI | 데이터 출처: Federal Reserve Economic Data</p>
    <p>⚠️ 본 대시보드는 투자 참고용이며, 투자 권유가 아닙니다.</p>
    <p>💡 AI 분석은 Google Gemini 2.0 Flash를 사용합니다 (무료)</p>
</div>
""", unsafe_allow_html=True)
